    ...
    async function fetchProducts() {
      ...
      if (error) console.error(error)
        ...
        const productsWithImages = data.map((p: Product) => ({
          ...p,
          imageUrl: `https://xxnkqekirvdjruvpwjjq.supabase.co/storage/v1/object/public/product_images/${p.slug}.png`
        }))
...
if (authLoading) {
  return (
    <div className={styles.authLoadingWrap} role="status" aria-live="polite">
      <div className={styles.authLoadingBox}>
        <div className={styles.authLoadingBar}>
          <div className={styles.authLoadingFill} />
        </div>

        <p className={styles.authLoadingText}>Checking authentication...</p>
      </div>
    </div>
  )
}
      ...
      {modalOpen && (
        <div className={styles.modalOverlay}>
          <form onSubmit={handleSubmit} className={styles.modalContent}>
            ...
            <input type="file" accept=".jpg, .jpeg, .png, .webp" onChange={e => e.target.files && setImageFile(e.target.files[0])} />
